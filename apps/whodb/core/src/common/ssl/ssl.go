...
// Content keys: Used when callers send certificate content directly.
...
// Content: Used when callers send certificate content via API
// Path: Used by profile-based connections only (server reads file directly)
type CertificateInput struct {
	Content string // PEM content directly (from API callers or inline in profile)
...
// SSLModeInfo provides metadata for a single SSL mode.
...
// This is used by clients to know which alternative names are accepted.
