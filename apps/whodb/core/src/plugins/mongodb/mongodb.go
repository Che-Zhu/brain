...
// GetColumnConstraints retrieves MongoDB schema validation rules and maps them
// to the shared constraint format. Supports $jsonSchema validator with:
...
func (p *MongoDBPlugin) ClearTableData(config *engine.PluginConfig, schema string, storageUnit string) (bool, error) {
	...
	}).Info("Cleared MongoDB collection")
