...
func TestInitializeRouterDoesNotServeStaticFallback(t *testing.T) {
	schema := graphapi.NewExecutableSchema(graphapi.Config{Resolvers: &graphapi.Resolver{}})
	r := InitializeRouter(schema, nil, nil, nil)
	...
	if rr.Code == http.StatusOK {
		t.Fatalf("expected root path not to serve static content")
	}
}

func TestInitializeRouterDoesNotRegisterRESTCompatibilityWrappers(t *testing.T) {
	schema := graphapi.NewExecutableSchema(graphapi.Config{Resolvers: &graphapi.Resolver{}})
	r := InitializeRouter(schema, nil, nil, nil)

	req := httptest.NewRequest(http.MethodGet, "/api/profiles", nil)
	credentials := source.Credentials{SourceType: "Postgres", Values: map[string]string{"Hostname": "localhost"}}
	...
	if rr.Code != http.StatusNotFound {
		t.Fatalf("expected /api/profiles to be absent, got %d", rr.Code)
	}
}
