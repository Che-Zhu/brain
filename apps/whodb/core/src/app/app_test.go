...
func TestRuntimeModeIsServer(t *testing.T) {
	if got := runtimeMode(); got != "server" {
		t.Fatalf("expected server mode, got %q", got)
	}
}
...
func TestResolvePortUsesEnvironmentOverride(t *testing.T) {
	t.Setenv("PORT", "")
	if got := resolvePort(); got != "11000" {
		t.Fatalf("expected default port 11000, got %q", got)
	}
