...
func TestAuthMiddlewareRejectsCookieWhenHeaderMissing(t *testing.T) {
	body := `{"operationName":"Other"}`
	creds := &source.Credentials{SourceType: "Postgres", Values: map[string]string{"Hostname": "localhost"}}
	...
	if rr.Code != http.StatusUnauthorized {
		t.Fatalf("expected unauthorized without Authorization header, got %d", rr.Code)
	}
}
...
func TestAuthMiddlewareRejectsIDOnlyCredentialsWithoutProfile(t *testing.T) {
	origEngine := src.MainEngine
	src.MainEngine = &engine.Engine{}
	t.Cleanup(func() { src.MainEngine = origEngine })

	id := "missing-profile-1"
	creds := source.Credentials{
		ID:     &id,
		Values: map[string]string{"Database": "override"},
	}
