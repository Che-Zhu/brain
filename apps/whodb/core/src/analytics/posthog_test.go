...
func TestBuildMetadataPrefersOriginAndStripsPort(t *testing.T) {
	...
	req.Header.Set("Origin", "https://client.local:3000")
	req.Header.Set("User-Agent", "test-agent")
	req.Header.Set("Referer", "https://client.local/dashboard")
	req.Header.Set("X-Request-Id", "req-123")

	metadata := BuildMetadata(req)

	if metadata.Domain != "client.local" {
	...
	if metadata.UserAgent != "test-agent" || metadata.Referer != "https://client.local/dashboard" {
