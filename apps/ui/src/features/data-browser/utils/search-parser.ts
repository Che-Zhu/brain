...
function parseAtomicCondition(search: string): ParsedSearch {
  ...
  const columnPattern =
    /^([a-zA-Z_][a-zA-Z0-9_]*)\s*(=|!=|<>|>=|<=|>|<|LIKE|NOT\s+LIKE|IN|NOT\s+IN|IS|IS\s+NOT)\s*(.+)$/i;
 ...
 */
function splitByLogicalOperators(
  search: string
): { operator: "AND" | "OR"; parts: string[] } | null {
...
export function parseSearchToWhereCondition(
  ...
  if (
    parsed.type === "compound" &&
    parsed.conditions &&
    parsed.compoundOperator
  ) {
    ...
    for (const condition of parsed.conditions) {
      if (
        condition.type === "column-specific" &&
        condition.column &&
        condition.operator &&
        condition.value
      ) {
        ...
        if (atomicCondition) {
          ...
          return createFullTextCondition(
            parsed.rawValue!,
            columns,
            columnTypes
          );
    ...
    if (atomicConditions.length === 1) {
      return atomicConditions[0] ?? undefined;
    }
  ...
  if (
    parsed.type === "column-specific" &&
    parsed.column &&
    parsed.operator &&
    parsed.value
  ) {
    return (
      createAtomicCondition(
        parsed.column,
        parsed.operator,
        parsed.value,
        columns,
...
function createAtomicCondition(
  ...
  const columnIndex = columns.findIndex(
    (col) => col.toLowerCase() === trimmedColumn.toLowerCase()
  );
...
function createOrCondition(
  ...
  const conditions: WhereCondition[] = columns.map((column) => {
