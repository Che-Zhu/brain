...
function getPrecedingKeyword(textBeforeCursor: string): string {
  ...
  for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    if (
      TABLE_CONTEXT_KEYWORDS.has(upper) ||
      COLUMN_CONTEXT_KEYWORDS.has(upper)
    ) {
      return upper;
    }
  }
...
function getReferencedTables(
  ...
  const fromPattern =
    /\bFROM\s+([\s\S]+?)(?=\b(?:WHERE|JOIN|INNER|LEFT|RIGHT|FULL|CROSS|ORDER|GROUP|HAVING|LIMIT|UNION|ON|SET|VALUES|INTO)\b|;|$)/gi;
  let fromMatch;
  while ((fromMatch = fromPattern.exec(fullText)) !== null) {
    ...
    for (const part of fromBody.split(",")) {
