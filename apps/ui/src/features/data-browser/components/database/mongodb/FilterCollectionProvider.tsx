...
const FilterCollectionCtx = createContext<FilterCollectionCtxValue | null>(
  null
);
...
export function useFilterCollectionCtx(): FilterCollectionCtxValue {
  ...
  if (!ctx) {
    throw new Error(
      "useFilterCollectionCtx must be used within FilterCollectionProvider"
    );
  }
  return ctx;
}
...
function draftValueFromPrimitive(
...
function parseInitialFilter(
  initialFilter: FlatMongoFilter | undefined
): ParsedFilterResult {
  ...
  for (const [field, rawValue] of Object.entries(initialFilter)) {
    ...
    if (regexValue !== undefined || regexOptions !== undefined) {
      const onlyRegexKeys =
        entries.length === 2 &&
        Object.hasOwn(rawValue, "$regex") &&
        Object.hasOwn(rawValue, "$options");
    ...
    const [operatorKey, operatorValue] = firstEntry;
    ...
    if (operator === "$in") {
      if (
        !(
          ...
          operatorValue.every((entry) => isPrimitive(entry))
        )
      ) {
        hasUnsupported = true;
        continue;
      ...
      parsed.push({
        ...
        operator,
        value: operatorValue
          .map((entry) => draftValueFromPrimitive(entry))
          .join(", "),
      });
      continue;
    }
  ...
  for (const condition of conditions) {
    ...
    const operatorValue =
      condition.operator === "$in"
        ? parseInDraftValue(condition.value)
        : parseDraftToken(condition.value);
...
export function FilterCollectionProvider({
  ...
  const addCondition = useCallback(() => {
    const nextField =
      findFirstUnusedField(fields, conditions) ?? fields[0] ?? null;
    if (!nextField) {
      pushAlert({
        type: "info",
  ...
  const updateCondition = useCallback(
    (id: string, updates: Partial<FilterConditionDraft>) => {
      setConditions((prev) =>
        prev.map((condition) =>
    ...
    >
      ...
      >
        <AlertBridge
          ...
          pendingAlertRef={pendingAlertRef}
        >
          {children}
        </AlertBridge>
      </ModalForm.Provider>
