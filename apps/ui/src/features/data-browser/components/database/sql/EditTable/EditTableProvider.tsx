...
import type { ModalMeta } from "@data-browser/components/ui/types";
import {
...
import {
  ...
  indexesQuery,
  modifyColumnSQL,
} from "@data-browser/utils/ddl-sql";
...
export function useEditTable(): EditTableContextValue {
  ...
  if (!ctx) {
    throw new Error("useEditTable must be used within EditTableProvider");
  }
  return ctx;
}
...
export function resolveForeignKeyDropName(
  ...
  return (
    originalForeignKeys.find(
      (originalForeignKey) => originalForeignKey.id === foreignKey.id
...
function EditTableBridge({
  ...
  const [originalForeignKeys, setOriginalForeignKeys] = useState<
    ForeignKeyDefinition[]
  >([]);
  ...
  const fetchTableSchema = async () => {
    ...
    try {
      ...
      const { data: colData } = await rawExecute({
        variables: {
          query: columnsQuery(dialect, databaseName, tableName, schema),
        },
        context: { database: databaseName },
      });
      if (colData?.RawExecute) {
        const colNames = colData.RawExecute.Columns.map((c) =>
          c.Name.toLowerCase()
        );
        const cols: ColumnDefinition[] = colData.RawExecute.Rows.map(
          (row, i) => {
            ...
            return {
              ...
              isPrimaryKey:
                (get("column_key") || get("pk")) === "PRI" || get("pk") === "1",
              isNullable:
                (get("is_nullable") || get("notnull")) !== "NO" &&
                get("notnull") !== "1",
              isNew: false,
            };
          }
      ...
      const { data: idxData } = await rawExecute({
        variables: {
          query: indexesQuery(dialect, databaseName, tableName, schema),
        },
        context: { database: databaseName },
      });
      if (idxData?.RawExecute) {
        const idxNames = idxData.RawExecute.Columns.map((c) =>
          c.Name.toLowerCase()
        );
        const idxs: IndexDefinition[] = idxData.RawExecute.Rows.map(
          (row, i) => {
            const get = (name: string) => row[idxNames.indexOf(name)] ?? "";
            return {
              id: `idx_${i}`,
              name: get("index_name") || get("name"),
              columns: (get("columns") || get("column_name") || "")
                .split(",")
                .filter(Boolean),
      ...
      const { data: fkData } = await rawExecute({
        variables: {
          query: foreignKeysQuery(dialect, databaseName, tableName, schema),
        },
        context: { database: databaseName },
      });
      if (fkData?.RawExecute) {
        const fkNames = fkData.RawExecute.Columns.map((c) =>
          c.Name.toLowerCase()
        );
        const fks: ForeignKeyDefinition[] = fkData.RawExecute.Rows.map(
          (row, i) => {
  ...
  const isColumnModified = useCallback(
    (col: ColumnDefinition): boolean => {
      const original = originalColumns.find((c) => c.id === col.id);
      if (!original) {
        return false;
      }
      return (
        col.type !== original.type || col.isNullable !== original.isNullable
      );
    },
    [originalColumns]
  );

  const isIndexModified = useCallback(
    (idx: IndexDefinition): boolean => {
  ...
  const isForeignKeyModified = useCallback(
    (fk: ForeignKeyDefinition): boolean => {
      const original = originalForeignKeys.find((f) => f.id === fk.id);
      if (!original) {
        return false;
      }
      return (
        fk.name !== original.name ||
  ...
  const pendingChangeCount = useMemo(() => {
    const colChanges =
      ...
      columns.filter((c) => !c.isNew && c.isMarkedForDeletion).length +
      columns.filter(
        (c) => !(c.isNew || c.isMarkedForDeletion) && isColumnModified(c)
      ).length;
    ...
    const idxChanges =
      ...
      indexes.filter((i) => !i.isNew && i.isMarkedForDeletion).length +
      indexes.filter(
        (i) => !(i.isNew || i.isMarkedForDeletion) && isIndexModified(i)
      ).length;
    ...
    const fkChanges =
      ...
      foreignKeys.filter((f) => !f.isNew && f.isMarkedForDeletion).length +
      foreignKeys.filter(
        (f) => !(f.isNew || f.isMarkedForDeletion) && isForeignKeyModified(f)
      ).length;
  ...
  const toggleColumnDeletion = (col: ColumnDefinition) => {
    if (col.isNew) {
      ...
      setColumns((prev) =>
        prev.map((c) =>
          c.id === col.id
            ? { ...c, isMarkedForDeletion: !c.isMarkedForDeletion }
            : c
        )
      );
    }
  ...
  const toggleIndexDeletion = (idx: IndexDefinition) => {
    if (idx.isNew) {
      ...
      setIndexes((prev) =>
        prev.map((i) =>
          i.id === idx.id
            ? { ...i, isMarkedForDeletion: !i.isMarkedForDeletion }
            : i
        )
      );
    }
  ...
  const updateForeignKey = (
    ...
    setForeignKeys((prev) =>
      prev.map((fk) => (fk.id === id ? { ...fk, [field]: value } : fk))
    );
  };
  ...
  const toggleForeignKeyDeletion = (fk: ForeignKeyDefinition) => {
    if (fk.isNew) {
      ...
      setForeignKeys((prev) =>
        prev.map((f) =>
          f.id === fk.id
            ? { ...f, isMarkedForDeletion: !f.isMarkedForDeletion }
            : f
        )
      );
    }
  ...
  const applyAllChanges = async () => {
    ...
    const newForeignKeys = foreignKeys.filter(
      (f) => f.isNew && !f.isMarkedForDeletion
    );
    ...
    const fksToDelete = foreignKeys.filter(
      (f) => !f.isNew && f.isMarkedForDeletion
    );
    ...
    const idxsToDelete = indexes.filter(
      (i) => !i.isNew && i.isMarkedForDeletion
    );
    ...
    const colsToDelete = columns.filter(
      (c) => !c.isNew && c.isMarkedForDeletion
    );
    ...
    const colsToModify = columns.filter(
      (c) => !(c.isNew || c.isMarkedForDeletion) && isColumnModified(c)
    );
    for (const col of colsToModify) {
      const sql = modifyColumnSQL(
        dialect,
        tableName,
        col.name,
        col.type,
    ...
    const idxsToModify = indexes.filter(
      (i) => !(i.isNew || i.isMarkedForDeletion) && isIndexModified(i)
    );
    for (const idx of idxsToModify) {
      ...
      const createSql = createIndexSQL(
        dialect,
        tableName,
        idx.name,
        idx.columns,
    ...
    const fksToModify = foreignKeys.filter(
      (f) => !(f.isNew || f.isMarkedForDeletion) && isForeignKeyModified(f)
    );
  ...
  const columnNames = useMemo(
    () =>
      columns
        .filter((c) => c.name.trim() && !c.isMarkedForDeletion)
        .map((c) => c.name),
    [columns]
  );
