  ...
  switch (action.type) {
    ...
    case "commit-active-cell": {
      ...
      if (current?.type === "insert") {
        ...
        const nextChange = upsertCellChange(
          current,
          action.originalRow,
          action.column,
          action.value
        ...
        if (nextChange) {
          nextChanges.set(action.rowKey, nextChange);
        } else {
    ...
    case "add-row": {
      ...
      return {
        ...
        undoStack: [
          ...state.undoStack,
          { kind: "add-row", rowKey: action.rowKey },
        ],
      };
    }
    ...
    case "delete-selected": {
      ...
      const previousChanges = action.rows.map(
        (row) => [row.rowKey, nextChanges.get(row.rowKey)] as const
      );
      ...
      for (const row of action.rows) {
        if (row.isInserted) {
          ...
          if (index >= 0) {
            nextNewRowOrder.splice(index, 1);
          }
          continue;
        }
      ...
      return {
        ...
        undoStack: [
          ...
          }),
          {
            kind: "delete-rows",
            rowKeys,
            previousChanges: previousChanges.map((entry) => [
              entry[0],
              entry[1],
            ]),
          },
    ...
    case "undo": {
      ...
      if (lastEntry.kind === "cell") {
        const reverted = revertCellUndo(
          nextChanges.get(lastEntry.rowKey),
          lastEntry
        );
        if (reverted) {
          nextChanges.set(lastEntry.rowKey, reverted);
        } else {
      ...
      if (lastEntry.kind === "add-row") {
        ...
        return {
          ...
          newRowOrder: state.newRowOrder.filter(
            (rowKey) => rowKey !== lastEntry.rowKey
          ),
          selectedRowKeys: new Set(
    ...
    case "prune-successes": {
      ...
      for (const entry of state.undoStack) {
        ...
        const nextRowKeys = entry.rowKeys.filter(
          (rowKey) => !action.rowKeys.includes(rowKey)
        );
...
function buildEmptyRowValues(columns: string[]) {
  return Object.fromEntries(
    columns.map(
      (column) => [column, null] satisfies [string, ChangesetCellValue]
    )
  );
}
...
function isEditableCell(
  ...
  if (!row.isInserted && primaryKey && column === primaryKey) {
    return false;
  }
  return true;
}
...
export function useChangesetManager({
  ...
  const commitCellValue = useCallback(
    (rowKey: ChangesetRowKey, column: string, value: string) => {
  ...
  const activateCell = useCallback(
    (rowKey: ChangesetRowKey, column: string) => {
      if (
        state.activeCell &&
  ...
  const moveActiveCell = useCallback(
    (direction: "left" | "right" | "up" | "down") => {
      ...
      const currentRowIndex = renderedRows.findIndex(
        (row) => row.rowKey === state.activeCell?.rowKey
      );
      const currentColumnIndex = visibleColumns.indexOf(
        state.activeCell.column
      );
