import type { ChangesetCellValue, ChangesetRowKey, RowChange } from "./types";
...
function buildWhereClause(row: Record<string, ChangesetCellValue>) {
  return Object.entries(row)
    .map(
      ([column, value]) => `${quoteIdentifier(column)} = ${quoteLiteral(value)}`
    )
    .join(" AND ");
}
...
export function buildPreviewSql(
  ...
  return [...changes.values()].map((change) => {
    ...
    const setClause = Object.entries(change.cells)
      .map(
        ([column, delta]) =>
          `${quoteIdentifier(column)} = ${quoteLiteral(delta.new)}`
      )
      .join(", ");
