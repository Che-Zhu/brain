...
function AccordionWithBorders() {
  ...
  return (
    <Preview title="With borders">
      <Accordion className="mx-auto max-w-lg gap-2">
        {items.map((item) => (
          <AccordionItem
  ...
  const items = [
    {
      ...
      content: (
        <>
          ...
          <p>
            <a href="https://example.com">Annual billing is available</a> with a
            20% discount. All plans include a 14-day free trial with no credit
            card required.
          </p>
          <Button size="sm">
            View plans
      ...
      <Accordion className="mx-auto max-w-lg overflow-hidden rounded-lg border">
        {items.map((item) => (
          <AccordionItem
            ...
            value={item.value}
          >
            <AccordionTrigger className="px-3">{item.trigger}</AccordionTrigger>
            <AccordionContent className="px-3">{item.content}</AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
    </Preview>
