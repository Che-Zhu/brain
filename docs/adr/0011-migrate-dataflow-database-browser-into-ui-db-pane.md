...
- Prefer existing `@workspace/ui` theme tokens and Tailwind classes for migrated DataBrowser styling. Add feature-local DataBrowser CSS only when a remaining migrated global style cannot be expressed locally in TSX without changing behavior. Host-owned canvas action surface utilities belong in the shared `@workspace/ui/globals.css` entrypoint with the rest of the canvas/resource-pane surface tokens.
...
- Use existing `@workspace/ui` color tokens for DataBrowser visual fitting where possible. Do not use the migration to retune component layout, spacing, density, radius, table structure, modal structure, or toolbar composition in TSX unless required for pane integration, compilation, or removal of migration-only CSS shims.
...
- Copy DataFlow global CSS wholesale into the app global stylesheet: rejected because it would leak DataFlow root variables, body rules, fonts, and deleted product styles into the rest of `apps/ui`. Host-owned canvas action surface utilities are separate from DataFlow migration CSS and can live in `@workspace/ui/globals.css`.
...
Because DataBrowser now relies on existing `@workspace/ui` tokens and local TSX classes where possible, the migrated browser can visually fit the brainv2 pane without carrying a migration-only theme stylesheet. Host canvas action surface color should use the existing canvas/resource-pane tokens from `@workspace/ui/globals.css`.
